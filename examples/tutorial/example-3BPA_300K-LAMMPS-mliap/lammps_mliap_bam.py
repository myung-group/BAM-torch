...
def rebuild_bam_mliap(pkl_path: str, backend: str = "e3nn",
    ...
    # enr_avg_per_element may be keyed by atomic number (omol/opoly-style
    # checkpoints) or by species index (BAM-torch base trainer, whose keys are
    # uniq_element's *values*).  Detect which convention this checkpoint uses.
    _ekeys = {int(k) for k in enr_raw}
    _zs = {int(z) for z in uniq}
    _ss = {int(s) for s in uniq.values()}
    _species_keyed = bool(_ekeys) and _ekeys == _ss and _ekeys != _zs
    if _species_keyed:
        print("  - enr_avg_per_element is species-indexed; using species keys.")

    if elements is not None:
        # explicit override: species s <-> elements[s]; reference energies are
        # keyed by atomic number unless the checkpoint is species-indexed
        sym_to_z = {chemical_symbols[z]: z for z in range(1, 119)}
        enr = {s: float(enr_raw.get(s if _species_keyed
                                    else sym_to_z.get(sym, -1), 0.0))
        ...
        for z, s in uniq.items():
            ...
            if 1 <= z < len(chemical_symbols) and 0 <= s < len(elements):
                ...
                enr[s] = float(enr_raw.get(s if _species_keyed else z, 0.0))
