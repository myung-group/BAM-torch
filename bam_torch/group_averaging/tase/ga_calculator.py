...
from bam_torch.group_averaging.predicting.ga_evaluator import GAEvaluator
from bam_torch.utils.utils import get_graphset_to_predict
from time import time


class GACalculator(Calculator, GAEvaluator):
    ...
    def calculate(self, atoms, properties=['energy'], system_changes=all_changes):
        ...
        print(f"Forward delta: {(after-before)/1024**2:.1f} MB | elapsed time: {time()-b_time} sec")
        ...
        self.results['forces'] = np.array(preds['forces'].detach().cpu()) #forces_grad_target
