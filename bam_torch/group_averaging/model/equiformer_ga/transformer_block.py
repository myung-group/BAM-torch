...
class TransBlockV2_(torch.nn.Module):
    """
...
class GraphAttentionNoEquiv(nn.Module):
    def __init__(
        self,
        node_channels,
        hidden_channels,
        ...
        #    self.src_emb = nn.Embedding(max_num_elements, edge_channels_list[-1])
        #    self.dst_emb = nn.Embedding(max_num_elements, edge_channels_list[-1])
        #    edge_channels_list[0] += 2 * edge_channels_list[-1]

        self.edge_mlp = RadialFunction(edge_channels_list)

        # projections
        self.q_proj = nn.Linear(node_channels, hidden_channels)
        self.k_proj = nn.Linear(node_channels, hidden_channels)
        self.v_proj = nn.Linear(node_channels, hidden_channels)

        self.out_proj = nn.Linear(hidden_channels, output_channels)
    ...
    def forward(self, x, atomic_numbers, edge_distance, edge_index):
        ...
        #    e = torch.cat([
        #        edge_distance,
        #        self.src_emb(atomic_numbers[src]),
        #        self.dst_emb(atomic_numbers[dst]),
        #    ], dim=1)
        #else:
        e = edge_distance
        edge_weight = self.edge_mlp(e)  # [E, hidden]
        ...
        # attention
        alpha = (Q * K).sum(dim=-1) / math.sqrt(self.head_dim)
        alpha = torch_geometric.utils.softmax(alpha, dst)
        alpha = self.alpha_dropout(alpha)

        msg = alpha.unsqueeze(-1) * V * edge_weight
        out = torch.zeros_like(x)
        out.index_add_(0, dst, msg)

        return self.out_proj(out)

class FFN_NoEquiv(nn.Module):
...
class AtomEmbedding(nn.Module):
    def __init__(self, max_num_elements, sphere_channels):
        super().__init__()
        self.net = nn.Embedding(max_num_elements, sphere_channels)
...
class TransBlockV2(nn.Module):
    def __init__(
        ...
        self.attn = GraphAttentionNoEquiv(
        ...
        self.ffn = FFN_NoEquiv(channels, ffn_hidden_channels)
    ...
    def forward(self, x, atomic_numbers, edge_distance, edge_index, batch):
        h = x
        x = self.norm1(x)
        x = self.attn(x, atomic_numbers, edge_distance, edge_index)

        if self.drop_path:
            x = self.drop_path(x, batch)
