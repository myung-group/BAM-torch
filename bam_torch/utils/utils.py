...
def get_graphset_with_pad(graphset, pad_nodes_to, pad_edges_to):
    ...
    for data in graphset:
        ...
        if n_nodes < pad_nodes_to:
            padding = torch.zeros((pad_nodes_to - n_nodes, data.pos.size(1)))
            data.positions = torch.cat([data.pos, padding], dim=0)
            data.species = torch.cat([data.atomic_numbers, padding[:, 0]], dim=0)
