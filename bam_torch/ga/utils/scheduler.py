    ...
    def __init__(self, optimizer, optim_config, silent=False):
        ...
        if (
            ...
            if self.scheduler_type == "ReduceLROnPlateau":
                #scheduler_params = {"mode": "min", "factor": 0.9, "patience": 10}
                #self.scheduler = self.scheduler(optimizer, **scheduler_params)
                self.scheduler = lr_scheduler.ReduceLROnPlateau(optimizer, 
                                                                factor=0.9, patience=50, threshold=1e-10)  # 15, 1e-4
            ###
